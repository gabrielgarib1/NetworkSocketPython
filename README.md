...
Handshake: The client initiates a connection with a SYN packet. The server responds with SYN+ACK, and the client finalizes with an ACK.

Data Transfer: The client sends data packets sequentially. For each packet, it waits for a corresponding ACK from the server before sending the next one.

Reliability: If an ACK is not received within the defined TIMEOUT, the client retransmits the packet. The server handles duplicate packets (by resending the ACK) and corrupted packets (by discarding them).

Teardown: The client sends a FIN packet to close the connection, and both sides exchange final ACKs.
